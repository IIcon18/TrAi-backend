...
class PostWorkoutTestCreate(BaseModel):
    tiredness: int = Field(
        ge=1, le=10, description="How tired do you feel after this workout? (1-10)"
    )
    mood: int = Field(ge=1, le=10, description="How was your overall mood? (1-10)")
    energy_level: int = Field(
        ge=1, le=10, description="How was your energy level during the session? (1-10)"
    )
    avg_rest_time: int = Field(
        ge=30, le=300, description="Average rest time between sets in seconds (30-300)"
    )
    completed_exercises: bool = Field(
        description="Did you complete all planned exercises?"
    )
    pain_discomfort: int = Field(
        ge=0, le=10, description="Any pain or discomfort during the workout? (0-10)"
    )
    performance: int = Field(
        ge=1, le=10, description="How would you rate your performance today? (1-10)"
    )
